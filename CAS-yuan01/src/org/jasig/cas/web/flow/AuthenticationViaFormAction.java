...
public class AuthenticationViaFormAction {
    ...
    public final String submit(final RequestContext context, final MessageContext messageContext) throws Exception {
        ...
        try {
            ...
            populateErrorsInstance(e, messageContext, context);
            return "error";
        } 
    }
    ...
    private void populateErrorsInstance(final AuthenticationException e, final MessageContext messageContext, final RequestContext context) {

        try {
        	String code = "error.authentication.credentials.bad";
        	MessageResolver message = new MessageBuilder().error().code(code).defaultText(code).build();
            messageContext.addMessage(message);
            context.getFlowScope().put("remoteLoginMessage", "用户名或密码错误!");  
        } catch (final Exception fe) {
            logger.error(fe.getMessage(), fe);
        }
    }
